...
const getDefaultBlocks = () => defaultBlocks.map((b) => ({ ...b, settings: { ...(b.settings || {}) } }));

const romanticDefaultData = {
  base: {
    names: 'María & Carlos',
...
export const useBuilderStore = defineStore('builderStore', {
  ...
  actions: {
    createDraftInvitation(template = null) {
      ...
      const isRomantic = template?.id === 'romantic-01';
      const invitation = {
        ...
        base: isRomantic ? { ...romanticDefaultData.base } : { names: '', date: '', location: '', heroMessage: '', storyMessage: '' },
        ...
        timeline: isRomantic ? romanticDefaultData.timeline.map((t) => ({ ...t })) : [],
        gallery: isRomantic ? romanticDefaultData.gallery.map((g) => ({ ...g })) : [],
        mapSettings: isRomantic ? { ...romanticDefaultData.mapSettings } : { locationName: '', address: '', mapUrl: '', embedUrl: '' },
        expiresAt, createdAt,
      };
    ...
    ensureBlocks() { if (!this.invitation) return []; if (!Array.isArray(this.invitation.blocks) || this.invitation.blocks.length === 0) this.invitation.blocks = getDefaultBlocks(); return this.invitation.blocks; },
    toggleBlock(blockType, enabled = null) { const blocks = this.ensureBlocks(); const b = blocks.find((it) => it.type === blockType); if (!b) return; b.enabled = typeof enabled === 'boolean' ? enabled : !b.enabled; },
