...
## HU-109 Romantic Motion layout polish
- Improved only the Romantic Motion template layout and responsive behavior.
- Added a shared `.romantic-template__container` so header, hero, story, countdown, details, gallery, map/FAQ, RSVP, and footer now align to consistent horizontal margins.
- Enlarged the desktop hero image with a two-column hero grid and intentional overlap into the following section.
- Updated detail cards so titles stay one line on desktop (with ellipsis fallback) and wrap naturally on mobile.
...
## HU-109B safe Romantic Motion layout fixes
- Unified side margins with a safe shared container (`min(calc(100% - 48px), 1120px)` desktop and `min(calc(100% - 28px), 100%)` mobile).
- Increased the hero first image size using safe min-height rules, without negative margins or overlap tricks.
- Tuned detail title sizing for safer desktop one-line behavior and natural wrapping on mobile.
- Added a safe details image reveal layer effect (no sticky/fixed behavior) using transform/opacity transitions and a soft overlay.
- Added focused mobile fixes for section stacking, hero typography, section vertical rhythm, and overflow safety.
